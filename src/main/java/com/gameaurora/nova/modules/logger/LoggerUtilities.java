...
public class LoggerUtilities {
	...
	private static String format(String string, LogEntryType type) {
		...
		if (type == LogEntryType.CHAT) {
			return "[CHAT] [" + dateFormat.format(date) + "] " + ChatColor.stripColor(string);
		}
		
		if (type == LogEntryType.COMMAND) {
			return "[COMMAND] [" + dateFormat.format(date) + "] " + ChatColor.stripColor(string);
		}
		return "";
	}
